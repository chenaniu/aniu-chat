...
HiChat.prototype = {  
    init: function() {//此方法初始化程序  
		...
		this.socket.on('nickExisted', function() {  
			document.getElementById('info').textContent = '昵称已经存在！'; //显示昵称被占用的提示  
		});
	...
	_initialEmoji: function() {  
		...
		for (var i = 1; i <= 5; i++) {  
			...
			emojiItem.src = '../content/emoji/emoji' + i + '.png';  
			emojiItem.title = i;  
	...
	_showEmoji: function(msg) {  
		...
		while (match = reg.exec(msg)) {  
			...
			if (emojiIndex > totalEmojiNum) {  
				...
				result = result.replace(match[0], '<img class="emoji" src="../content/emoji/emoji' + emojiIndex + '.png" />');  
			};  
		};  
		return result;  
	}  	
